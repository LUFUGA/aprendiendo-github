...
--	 PK_VIVIENDA_CAWI_VC like (SELECT TOP 1 CodigoAcceso+'%'  FROM GES_INTEGRACION_QR_SUSO  WHERE CodUsoDestino =  C.CodUsoDestino order by FechaSincronizacion desc)
	 AND (PER_ESTADO_PERSONA = 1  or PER_ESTADO_PERSONA_RPH = 1)
	)AS RESIDENTES_CENSADOS,
	len((SELECT TOP 1 CodigoAcceso  FROM GES_INTEGRACION_QR_SUSO  WHERE CodUsoDestino = C.CodUsoDestino order by FechaSincronizacion desc)) as largo
FROM GES_ASIGNACION_COLECTIVAS  C
--WHERE Tipo_Levantamiento in (1,2,3)
WHERE Tipo_Levantamiento = 3
--and c.CodUsoDestino = '4015861'
--and len((SELECT TOP 1 CodigoAcceso  FROM GES_INTEGRACION_QR_SUSO  WHERE CodUsoDestino = C.CodUsoDestino order by FechaSincronizacion desc))>5



SELECT TOP 100 
--CodigoAcceso 
CodUsoDestino  , COUNT(1)
FROM GES_INTEGRACION_QR_SUSO  --WHERE CodUsoDestino = C.CodUsoDestino 
group by CodUsoDestino --CodigoAcceso
having COUNT(1)>1
--order by FechaSincronizacion desc

SELECT 
ROW_NUMBER() OVER(PARTITION BY CodUsoDestino ORDER BY FechaSincronizacion DESC) AS Row#,
CodUsoDestino, CodigoAcceso, FechaSincronizacion, LEN(CodigoAcceso)--, *
FROM GES_INTEGRACION_QR_SUSO  
WHERE CodUsoDestino = '1156311'
--AND ROW_NUMBER()=1
